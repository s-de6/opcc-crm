...
export default function FileStorage() {
  ...
  const uploadMut = useMutation({
    ...
    onSuccess: async (data: any) => {
      ...
      try {
        ...
        if (importResp.status === 409 || result?.error === 'Statement already imported') {
          setDupWarning({
            show: true,
            fileId,
        ...
        // Issue 17: if scores are close (diff < 3) OR both are 0, ask user to choose
        if (scoreDiff < 3 || (bankScore === 0 && invoiceScore === 0)) {
          setTypeChoice({
            show: true,
            fileId,
        ...
        if (docType === 'bank_statement' && result?.statement_id) {
          ...
          // Invoice flow: no review page yet — show success and stay on File Storage
          const items = result?.items_count != null ? ` · ${result.items_count} line item(s)` : '';
          const folder = result?.partner_folder ? `\n\nSaved to folder: ${result.partner_folder}` : '';
          alert(`Invoice imported successfully${items}.${folder}\n\nGo to the Reconciliation page to match this invoice against bank transactions.`);
          queryClient.invalidateQueries({ queryKey: ['file-storage'] });
          queryClient.invalidateQueries({ queryKey: ['file-storage-folders'] });
  ...
  const handleDupChoice = async (reupload: boolean) => {
    ...
    const { fileId, statementId, pendingFile } = dupWarning;
    setDupWarning(null);

    if (!reupload) {
      // User said No — navigate to existing statement
      if (statementId) navigate(`/bank-statements/review/${statementId}`);
      return;
    }
    ...
    // Post-upload duplicate — re-import with force=true
    setProcessingMsg('Re-importing bank statement…');
    try {
      const token = localStorage.getItem('token') || '';
      const activeClient = localStorage.getItem('activeClient');
  ...
  const handleTypeChoice = async (choice: 'bank_statement' | 'invoice' | 'store') => {
    ...
    try {
      ...
      if (choice === 'bank_statement' && result?.statement_id) {
        ...
        const items = result?.items_count != null ? ` · ${result.items_count} line item(s)` : '';
        alert(`Invoice imported successfully${items}.\n\nGo to the Reconciliation page to match it against bank transactions.`);
        queryClient.invalidateQueries({ queryKey: ['file-storage'] });
      } else if (result?.error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Duplicate bank statement warning popup */}
      {dupWarning?.show && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center backdrop-blur-sm">
